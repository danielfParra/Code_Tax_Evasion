...
doc = """
Random Taxpayers - Tax Evasion Game
...
class Player(BasePlayer):
    ...
    q3TP = models.IntegerField(
        choices=[
            ...
            [3, 'The {} that Player A can win correspond to the collected taxes coming '
                'from your group\'s earnings in the practice round in Part 1.'.format(Constants.money_to_take)],
        ],
        widget=widgets.RadioSelect,
    )

    q4TP = models.IntegerField(
        choices=[
            [1, 'Yes, they could choose not to participate.'],
            [2, 'No, they could not choose whether to participate or not.'],
        ],
        widget=widgets.RadioSelect,
    )
...
def q4TP_error_message(player, value):
    print('value is', value)
    if value != 2:
        return 'Recall: All the Participants A were included in the lottery. They did not decide to participate in it.'


# PAGES

class Inst_RandomTP(Page):
    @staticmethod
...
page_sequence = [Inst_RandomTP, Control_Q_TP, Decision_TP, Takewheel, Feedback_TP]
